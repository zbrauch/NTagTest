 ...
 * Constructor.
 */
MFRC522::MFRC522(SPI_HandleTypeDef *spi, void (*EnableCSFunc)(void), void (*DisableCSFunc)(void)) {
	this->SPIHandle = spi;
...
void MFRC522::PCD_WriteRegister(uint8_t reg_addr, uint8_t write_data) {
	EnableCS();
	uint8_t transmit[2];
	...
	uint8_t ret;
	HAL_SPI_Transmit(SPIHandle, transmit, 2, HAL_MAX_DELAY);
...
void MFRC522::PCD_WriteRegister(uint8_t reg_addr, uint8_t write_len, uint8_t *write_data) {
 ...
 * The interface is described in the datasheet section 8.1.2.
 */
uint8_t MFRC522::PCD_ReadRegister(uint8_t reg_addr) {
	EnableCS();
...
void MFRC522::PCD_ReadRegister(uint8_t reg_addr, uint8_t count, uint8_t *values, uint8_t rxAlign) {
	if(count == 0)
		return;
 ...
 */
//Set bits in mask
void MFRC522::PCD_SetRegisterBitMask(uint8_t reg_addr, uint8_t mask) {
	return PCD_WriteRegister(reg_addr, PCD_ReadRegister(reg_addr) | mask);
}
...
void MFRC522::PCD_ClearRegisterBitMask(uint8_t reg_addr, uint8_t mask) {
	return PCD_WriteRegister(reg_addr, PCD_ReadRegister(reg_addr) & (~mask));
}
...
void MFRC522::PCD_Reset() {
	...
	uint8_t count = 0;
	do {
		// Wait for the PowerDown bit in CommandReg to be cleared (max 150ms)
		osDelay(10);
	} while ((PCD_ReadRegister(CommandReg) & (1 << 4)) && (++count) < 15);
} // End PCD_Reset()
...
void MFRC522::PCD_SoftPowerUp(){
	...
	uint32_t timeout = 500;// create timer for timeout (just in case)

	while(timeout > 0){ // set timeout to 500 ms
